...
class Node:
    ...
    def is_move_valid(self, grid_map, move):
        ...
        if goal_pixel[0] >= SAFETY_OFFSET and not grid_map[goal_pixel[1]-SAFETY_OFFSET][goal_pixel[0]]:
            return False
        if goal_pixel[1] >= SAFETY_OFFSET and not grid_map[goal_pixel[1]][goal_pixel[0]-SAFETY_OFFSET]:
            return False
        if goal_pixel[0] >= SAFETY_OFFSET and goal_pixel[1] >= SAFETY_OFFSET and not grid_map[goal_pixel[1]-SAFETY_OFFSET][goal_pixel[0]-SAFETY_OFFSET]:
            return False
        if grid_map[goal_pixel[1]][goal_pixel[0]]:
            return True
        return False
